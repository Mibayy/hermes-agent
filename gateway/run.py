...
class GatewayRunner:
    ...
    async def _run_agent(
        ...
        # RBAC: apply per-user tool restrictions from gateway.user_roles.
        try:
            from gateway.config import load_gateway_config as _load_gw_cfg
            _gw_cfg = _load_gw_cfg()
            _user_role = _gw_cfg.get_user_role(source.user_id)
            _filtered = _user_role.apply(enabled_toolsets)
            if _filtered != enabled_toolsets:
                logger.info(
                    "[RBAC] user=%s role=%s: toolsets restricted %s -> %s",
                    source.user_id, _user_role.role, enabled_toolsets, _filtered,
                )
                enabled_toolsets = sorted(_filtered)
        except Exception as _rbac_err:
            logger.debug("[RBAC] Could not apply user role: %s", _rbac_err)
