...
class UserRole:
    """
    Per-user tool restriction rule.
    ...
    Example config.yaml::
        ...
        gateway:
          user_roles:
            ...
            default:
              role: viewer
              allowed_tools: [mcp__*]  # Only MCP-prefixed tools

    Precedence:
      - allowed_tools: allowlist (only these tools enabled); supports shell-style
        wildcards (e.g. mcp__*).  Special value all skips filtering.
      - blocked_tools: denylist applied on top of the platform toolsets.
      - If both are provided, allowed_tools takes precedence.
      - The default entry applies to any user not matched by user_id.
    """
    role: str = "viewer"
    # Comma/list of toolset names to allow; "all" means no restriction.
    allowed_tools: Any = None   # str | List[str] | None
    # Comma/list of toolset names to block.
    blocked_tools: Any = None   # str | List[str] | None
    ...
    def apply(self, toolsets: List[str]) -> List[str]:
        """Return the filtered toolset list after applying this role."""
        import fnmatch

        # ------------------------------------------------------------------
        # allowed_tools takes full precedence when present.
        # ------------------------------------------------------------------
        if self.allowed_tools is not None:
            if self.allowed_tools == "all" or self.allowed_tools == ["all"]:
        ...
        # ------------------------------------------------------------------
        # blocked_tools: remove matching entries.
        # ------------------------------------------------------------------
        if self.blocked_tools is not None:
            blocked_raw: List[str] = (
                [self.blocked_tools]
                if isinstance(self.blocked_tools, str)
                else list(self.blocked_tools)
            )
            result = []
            for ts in toolsets:
                blocked = any(fnmatch.fnmatch(ts, p) for p in blocked_raw)
                if not blocked:
                    result.append(ts)
            return result
...
class GatewayConfig:
    ...
    # Per-user RBAC tool restrictions
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        for uid, role_data in (data.get("user_roles") or {}).items():
            if isinstance(role_data, dict):
                user_roles[str(uid)] = UserRole.from_dict(role_data)
    ...
    def get_user_role(self, user_id: Optional[str]) -> "UserRole":
        """Return the UserRole for this user_id, falling back to 'default'."""
        if user_id and user_id in self.user_roles:
            return self.user_roles[user_id]
        if "default" in self.user_roles:
            return self.user_roles["default"]
        return UserRole()
