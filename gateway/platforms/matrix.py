...
Environment variables:
    ...
    MATRIX_REQUIRE_MENTION      Set "false" to respond to all messages (default: true)
    MATRIX_FREE_RESPONSE_ROOMS  Comma-separated room IDs that bypass require_mention
"""
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Require-mention in group rooms (default: true, like Discord).
        self._require_mention: bool = config.extra.get(
            "require_mention",
            os.getenv("MATRIX_REQUIRE_MENTION", "true").lower() not in ("false", "0", "no"),
        )
        # Room IDs that bypass require_mention (always respond without mention).
        _free_rooms_raw: str = config.extra.get(
            "free_response_rooms",
            os.getenv("MATRIX_FREE_RESPONSE_ROOMS", ""),
        )
        self._free_response_rooms: Set[str] = {
            r.strip() for r in _free_rooms_raw.split(",") if r.strip()
        }
        # Threads where the bot has already participated — no mention needed.
    ...
    async def _on_room_message(self, room: Any, event: Any) -> None:
        ...
        # Require-mention check: in group rooms, only respond if the bot was
        # mentioned — unless the room is in the free-response list or the
        # message is in a thread the bot has already participated in.
        if not is_dm and self._require_mention:
            is_free_room = room.room_id in self._free_response_rooms
            in_bot_thread = (
                thread_id is not None
                and thread_id in self._bot_participated_threads
            )
            if not is_free_room and not in_bot_thread:
                # Check m.mentions (MSC3952) first, then fall back to display-name match.
                source_mentions = source_content.get("m.mentions", {})
                mentioned_mxids = source_mentions.get("user_ids", [])
                bot_mentioned = self._user_id in mentioned_mxids
                if not bot_mentioned and self._user_id:
                    # Fallback: plain-text mention by display name or MXID.
                    localpart = self._user_id.split(":")[0].lstrip("@") if ":" in self._user_id else self._user_id.lstrip("@")
                    bot_mentioned = (
                ...
                if not bot_mentioned:
                    return
                # Strip the mention prefix from the body before processing.
                if self._user_id:
                    body = body.replace(self._user_id, "").strip()
                    localpart = self._user_id.split(":")[0].lstrip("@") if ":" in self._user_id else self._user_id.lstrip("@")
                    # Strip @localpart: style mentions too.
                    import re as _re
                    body = _re.sub(r"@" + _re.escape(localpart) + r"(?::[\w.-]+)?", "", body).strip()

        # Track thread participation so follow-up messages don't need a mention.
        if thread_id and thread_id not in self._bot_participated_threads:
