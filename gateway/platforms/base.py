...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Sessions where auto-queue mode is enabled (set by /queue on / /queue off)
        self._queue_mode_sessions: set = set()
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            if event.get_command() == "stop":
                ...
                if args in ("on", "off"):
                    if args == "on":
                        self._queue_mode_sessions.add(session_key)
                        print(f"[{self.name}] 🔀 Queue mode ON for session {session_key}")
                    else:
                        self._queue_mode_sessions.discard(session_key)
                        print(f"[{self.name}] ⚡ Queue mode OFF for session {session_key}")
                    return
                ...
                if session_key in self._queue_mode_sessions:
                    print(f"[{self.name}] 📨 Auto-queued message for active session {session_key} (queue mode on)")
                    self._append_pending_event(session_key, event)
                else:
        ...
        # Handle /queue on|off even when agent is not running
