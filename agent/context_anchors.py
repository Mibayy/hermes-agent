...
def build_anchor_save_prompt(anchor: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"with any new state from this session: what changed, what was fixed, "
        f"what's running, current status, and anything NOT to repeat. "
        f"Read the file first with read_file, then use patch to update only "
        f"what changed. Keep it factual and concise. Do NOT rewrite the whole file.]"
    )
